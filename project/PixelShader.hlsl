...
#ifndef __GGP_VERT_TO_PIXEL__
#define __GGP_VERT_TO_PIXEL__
#include "VertToPixStruct.hlsli"
#endif
...
float3 GetDirectionalLight(Light dl, float3 normal, float3 color, float3 worldPos, float3 toCamera, float specInt) {
	...
	return GetSpecular(normal, toLight, toCamera, specInt) * baseColor + baseColor;
	//return float3(0, 0, 0);
...
float3 GetPointLight(Light pl, float3 normal, float3 color, float3 worldPos, float3 toCamera, float specInt) {
	...
	return GetSpecular(normal, -lightDir, toCamera, specInt) * baseColor + baseColor + pl.ambientColor * color;
	//return float3(0, 0, 0);
}
